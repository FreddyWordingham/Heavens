...
pub fn nbody(particles: &mut [Particle], grav_strength: f32, smoothing_length: f32, dt: f32) {
    ...
    if cpu_buffer.iter().any(|&x| x.is_nan()) {
        panic!("NaN detected in GPU buffer");
    }

    for (n, particle) in particles.iter_mut().enumerate() {
        particle.vel += Vector3::new(
            cpu_buffer[(n * 3) + 0] as f32,
            cpu_buffer[(n * 3) + 1] as f32,
            cpu_buffer[(n * 3) + 2] as f32,
        );
...
//     let src = format!(
//         "
//         __kernel void nbody(__global float* buffer, __global float* positions, int iteration) {{
//             bool direction = iteration % 2 == 0;
//             if (direction) {{
//                 __global float* tmp = buffer;
//                 buffer = positions;
//                 positions = tmp;
//             }}
